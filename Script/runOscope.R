...
infile <- args[grep(x=args,pattern="--infile")+1]

outdir <- args[grep(x=args,pattern="--outdir")+1]

infile_path <- paste0(outdir,"/",infile)
outfile <- paste0("OscopeCluster_",infile)
# 
...
# 	Out <- DataSc2
# }
eigenD <- read.csv(infile_path)
OscopeData <- as.matrix(eigenD[,-c(1,2)])

dimnames(OscopeData) <- list("r"=eigenD[,1], "c"=colnames(eigenD)[-c(1,2)] )
...
temp <- quantile(SineRes$DiffMat, probs = seq(0, 1, by=0.1) )
...
for(i in 1:length(temp)){
	...
	for (j in 1:(rown-1) )  {
		for (k in (j+1):(rown) )  {
			if (similarD[k,j] <= cutoff)  {
				peigenName <- c(peigenName , paste(eigenID[j-1], eigenID[k-1], sep = "|") )
				peigenIndex <- c(peigenIndex, paste(sort(c(j-1,k-1)), collapse ="|"))
			}
		}
	}

	if(length(peigenName)==0){
		next
	}

	if(length(peigenName)>max){
		max <- length(peigenName)
		best_peigenName <- peigenName
		best_peigenIndex <- peigenIndex
	}

}


peigenName <- best_peigenName
peigenIndex <- best_peigenIndex
